...
void multiply(const int *mat_1, const int *mat_2, int *mat_prod, const int n, const int m, const int p, const int start, const int end) {
	...
	int sum = 0;
	for (int k = 0; k < m; k++) {
	    sum += mat_1[row*m+k] * mat_2[k*p+col];
	}	
	mat_prod[index] = sum;
    }
}
...
int main(void) {
    ...
    for (int i = 0; i < n_threads; i++) {
        ...
        threads[i] = std::thread(multiply, mat_1, mat_2, mat_prod, n, m, p, start, end);
    }
