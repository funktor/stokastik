...
def get_shared_model(max_words, vocab_size, embed_dim):
    ...
    nlayer = Conv1D(64, 3, activation='relu', padding='same')(nlayer) #None, 200, 32
    nlayer = Conv1D(64, 3, activation='relu', padding='same')(nlayer) #None, 200, 32
    nlayer = BatchNormalization()(nlayer)
    nlayer = MaxPooling1D(2)(nlayer) #None, 100, 32
    
    nlayer = Conv1D(128, 3, activation='relu', padding='same')(nlayer) #None, 100, 64
    nlayer = Conv1D(128, 3, activation='relu', padding='same')(nlayer) #None, 100, 64
    nlayer = BatchNormalization()(nlayer)
    nlayer = MaxPooling1D(2)(nlayer) #None, 50, 64
    
    nlayer = Conv1D(256, 3, activation='relu', padding='same')(nlayer) #None, 50, 64
    nlayer = Conv1D(256, 3, activation='relu', padding='same')(nlayer) #None, 50, 64
    nlayer = BatchNormalization()(nlayer)
    nlayer = MaxPooling1D(2)(nlayer) #None, 50, 64
...
class DNNFeatures():
    def __init__(self, model_path, embed_dim=128, max_words=200, batch_size=64, num_epochs=64, features=None):
    ...
    def init_model(self):
        ...
        merge = dot([nlayer1, nlayer2], axes=1)
    ...
    def fit(self, X, y=None):
        ...
        data_pairs = create_pairs(y)
        n = len(data_pairs)
        
        self.vocab_size = len(self.dnn_tokenizer.word_index)+1
        
        m = int(math.ceil(n/self.batch_size))
        ...
        self.model.fit_generator(data_generator(data_pairs, input_tensor, self.batch_size),
                                 callbacks=callbacks, 
                                 steps_per_epoch=m, 
