...
class Solution(object):
    ...
    def profit_windows(self, prices):
        windows = []
        start, end = 0, 1
        ...
        while end < len(prices):
            if prices[end] < prices[end - 1]:
                ...
                if prices[end - 1] - prices[start] > 0:
                    windows.append((start, end - 1, prices[end - 1] - prices[start]))
    ...
    def maxProfit(self, k, prices):
        ...
        windows = self.profit_windows(prices)
        ...
        for m in range(1, q+1):
            ...
            for n in range(m-1, len(windows)):
                a = cache[m - 1].get(n - 1, 0) + windows[n][2]
                b = cache[m - 1].get(curr_min - 1, 0) + prices[windows[n][1]] - prices[windows[curr_min][0]]
                c = max(a, b)

                if a > b:
                    curr_min = n

                if cache[m].get(n - 1, 0) >= c:
                    cache[m][n] = cache[m].get(n - 1, 0)
                else:
                    cache[m][n] = c

        return cache[q][len(windows) - 1]
