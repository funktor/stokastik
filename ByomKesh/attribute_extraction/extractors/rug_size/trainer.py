...
from attribute_extraction.extractors.rug_size.normalizer import Normalizer
...
print("Reading data...")
df = pd.read_csv('data/rug_size.csv')
...
true_labels = [re.sub(' ', '', x) for x in true_labels]
...
pred_labels = [re.sub(' ', '', x) for x in pred_labels]
...
pred_labels2 = [re.sub(' ', '', x) for x in pred_labels2]
