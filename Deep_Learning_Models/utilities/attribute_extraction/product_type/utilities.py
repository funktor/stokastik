...
def create_text_data():
    try:
        ...
        items = list(zip(titles, short_desc, long_desc))
        
        if cnt.VECTOR_MODEL == 'fasttext':
            shutils.train_fasttext_model(items, get_all_tokens_for_vector, cnt.FAST_TEXT_PATH_WORD, char_tokens=False)
        else:
            shutils.train_wv_model(items, get_all_tokens_for_vector, cnt.WORD_VECT_PATH_WORD, char_tokens=False)
    
        word_vector_model = get_vector_model(cnt.VECTOR_MODEL, char_tokens=False)

        txt_arr_file = tables.open_file(cnt.TEXT_ARRAY_PATH, mode='w')
        atom = tables.Float32Atom()
        txt_arr = txt_arr_file.create_earray(txt_arr_file.root, 'data', atom, (0, cnt.MAX_WORDS, cnt.WORD_VECTOR_DIM))

        batch_size = 5000
        num_batches = int(math.ceil(float(len(items))/batch_size))
        print(num_batches)
        ...
        for m in range(num_batches):
            print(m)
            start, end = m*batch_size, min((m+1)*batch_size, len(items))
            tokens = get_all_tokens_for_vector([items[i] for i in range(start, end)])
            
            vectors = shutils.get_vectors(word_vector_model, tokens, cnt.WORD_VECTOR_DIM)
            txt_arr.append(vectors)
    finally:
        txt_arr_file.close()
