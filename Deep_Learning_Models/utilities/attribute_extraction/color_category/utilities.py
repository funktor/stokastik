...
def create_text_data():
    ...
    class_labels = [x.strip().split('__') for x in list(df['color_category'])]
...
def create_train_test():
    try:
        ...
        train_indices, test_indices = train_test_split(range(img_arr.shape[0]), test_size=0.2)
...
def custom_classification_scores(true_labels_names, pred_labels_names):
    ...
    for idx in range(len(true_labels_names)):
        ...
        for label in pred_label:
            if label in true_label or (true_label[0] == 'Multicolor' and label != 'None'):
                tp[label] += 1
            elif true_label[0] == 'Multicolor' and label != 'None':
                tp[label] += float(4.0)/17.0
            else:
                fp[label] += 1
